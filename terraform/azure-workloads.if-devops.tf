resource "azuread_application_federated_identity_credential" "devops_workload" {
  ...
  issuer  = azuredevops_serviceendpoint_azurerm.workload[each.key].workload_identity_federation_issuer
  subject = azuredevops_serviceendpoint_azurerm.workload[each.key].workload_identity_federation_subject
}
