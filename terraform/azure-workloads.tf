  ...
  workload_environments = flatten([
    for environment_name, workload in var.workloads : [
      for environment in workload.environments : {
        ...
        devops_create_variable_group = environment.devops_create_variable_group || environment.add_deploy_script_identity // If we're adding a deploy script identity, we need a variable group
